...
The UI consists of the 5 main layout regions (Figure 2):
...
. Attendance List Panel: panel that lists all attendees in the selected event.
// end::uilayout[]
...
Attendance List Panel will display the persons registered for that event.
...
Registers the 2nd person in EPIC for AY201718 Graduation. See the figures below for UI changes. +

[#register]
.Before the `register` command is executed
image::RegisterUIBefore.png[width="790"]

.After the `register` command is executed
image::RegisterUIAfter.png[width="790"]
...
=== Listing all Event Participants: `find-attendance`

Finds registrants whose names contain any of the given keywords.

Format: `find-attendance KEYWORD [MORE_KEYWORDS]`

If your event contains too many participants, it can be difficult to find information quickly. The
`find-attendance` command can be used to narrow down participants by their name. Only persons whose
names contain any of the given keys is displayed.

In our example, we'll apply a filter to display only participants from the IoT Talk Event with the
name "Alex".

. Select the IoT Event by executing `select-event 1`.
. Execute `find-attendance alex`, this filters the attendance panel, hiding anyone whose name does
...
be computed based on the filtered list (see <<#after-find-attendance>>).



[#before-find-attendance]
.Before the `find-attendance alex` command is executed
image::before_find_attendance.png[width="790"]
[#after-find-attendance]
.After the `find-attendance alex` command is executed
image::after_find_attendance.png[width="790"]
...
* `delete 2` +
`undo` +
Undoes the `delete 2` command. See the figures below for UI changes. In particular,
<<#undoafter>> shows that relative order of persons might not be preserved. +

.Before the `delete 2` command is executed
image::DeleteUIBefore.png[width="790"]

.After the `delete 2` command is executed
image::UndoUIBefore.png[width="790"]

[#undoafter]
.After the `undo` command is executed
image::UndoUIAfter.png[width="790"]
