...
. Select the IoT Event by executing `select-event 1`. Observe that Alice Pauline is registered
. Execute `find-registrant alice`, this filters the attendance panel, hiding anyone whose name does
not contain the word alice (case-insensitive).
...
be computed based on the filtered list (see <<#after-find-registrant>>).



[#before-find-registrant]
.Before the `find-registrant alice` command is executed
image::before_find_attendance.png[width="750"]
[#after-find-registrant]
.After the `find-registrant alice` command is executed
image::after_find_attendance.png[width="750"]
...
the 2nd event currently listed. <<#ToggleBefore>> below shows an example of
how EPIC looks like after the `select-event 2` command is executed and
<<#ToggleAfter>> below shows how EPIC would look like after the `toggle 3` command
is executed.

[#ToggleBefore]
.GUI before the `toggle 3` command is executed
image::ToggleScreenShotBefore.png[width="800"]

[#ToggleAfter]
.GUI after the `toggle 3` command is exectued
image::ToggleScreenShotAfter.png[width="800"]
//end::toggle[]
