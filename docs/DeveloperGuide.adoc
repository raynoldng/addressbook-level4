...
:repoURL: https://github.com/CS2103T-2018-W13-B2/addressbook-level4
...
[NOTE]
Note how the `Model` simply raises a `AddressBookChangedEvent` when the Address Book data is changed, instead of asking the `Storage` to save the updates to the hard disk.
...
We will pop the most recent command out of the `undoStack` and push it back to the `redoStack`. We will restore the address book to the state before the `add` command is executed.
...
** Cons: Performance issues may result due to high memory usage.
* **Alternative 2:** Individual command knows how to undo/redo by itself.
** Pros: Significantly less memory is used (e.g. for `delete`, just save the person being deleted).
** Cons: Implementation is slightly more complicated.
...
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `eventPlanner.xml` so that the application will load the sample data when you launch it.)
...
. Fix `invalidAndValidPersonAddressBook.xml`, `typicalPersonsEventPlanner.xml`, `validAddressBook.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.
...
|`* *` |event planner in charge of marking attendance of many participants |find a participant by his/her initials |mark attendance quickly

|`* *` |event planner |manage participants based on tags |mass register/delete participants belonging to a certain group

|`* *` |event planner |add multiple contacts from a file |do without adding each contact manually
...
[discrete]
=== Use case: Mark person

*MSS*

1. User requests to find persons by name
2. EventPlanner shows a list of persons
3. User requests to mark that person
4. EventPlanner marks the person
+
Use case ends.
...
.  Should be able to hold up to 50000 persons without a noticeable sluggishness in performance for typical usage.
...
.  Should be able to run without relying on other dependencies
