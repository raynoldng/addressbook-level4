...
. Attendance List Panel: panel that lists all registrants in the user selected event.
. Person Card: card that lists a peron's contact information.
. Event Card: card that lists an event's details.
. Attendance Card: card that lists an registrant's contact information and displays if the registrant has attended or not.
...
returns a `FindRegistrantCommand` object. When the `FindRegistrantCommand` object is exceuted, the Model filters the
...
The new UI is a 3 pane UI consisting of list of persons, events, and registrants for the selected event. Being able to
...
In addition, `edit` and `edit-command` commands are now implemented in a mutable manner
 ...
 longer be notified to changes. The new UI must be notified of changes to data so it can refresh itself to reflect
...
- *Alternative 1 (current choice)*: A 3 pane UI consisting of list of persons, events, and registrants for selected event
...
- *Alternative 2 (previous choice)*: 2 pane UI where the left pane is a 2 tab pane consisting of list of persons and events, and the right pane is the list of registrants.
    * Pros: Merging the horizontal space for list of persons and events will create more space for list of registrants. The user is likely to be more interested in the registrants details.
...
- *Alternative 3*: A common list that can display either list of persons, events or registrants for selected event
