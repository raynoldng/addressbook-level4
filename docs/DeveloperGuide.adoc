...
* It binds itself to some data in the Model component so that the it can auto-update itself when data in the Model component changes.
...
. Attendance List Panel: panel that lists all attendees in the user selected event.
...
The implementation uses a class called `Attendance`.
...
to have access to all its attendees so that adding, removing and listing attendees are
easy to implement.
...
whose names contain any of the given keywords will be listed. The user can further toggle the attendance of the registants with the `toggle` command.

<<#SDSEC>> below shows how the `find-registrant` command is processed in the Logic component.

[#SDSEC]
.Seqeuence Diagram for `find-registrant` command
image::FindAttendanceSequenceDiagram.png[width="650"]

==== Current Implementation

The `find-registrant` command is passed to the `EventPlannerParser` object when then reads the parses the input and returns a `FindRegistrantCommand` object. When the `FindRegistrantCommand` object is exceuted, the Model filters the list of registrants and the Attendance List Panel that is bind to the list of registrants is updated.
...
The new UI is a 3 pane UI consisting of list of persons, events, and attendees for the selected event. Being able to view all three panes will allow the user to read off the desired command parameters without having to switch between lists.

In addition, `edit` and `edit-command` commands are now implemented in a mutable manner (see <<edit-curr-implementation>>). As a result, the `ObservableList` that wraps the people and event data will no longer be notified to changes. The new UI must be notified of changes to data so it can refresh itself to reflect such changes. The UI will also have to respond to new commands such as the `select-event` and `toggle`. The `select-event` could invoke a change in the contents of the Attendance Panel and `toggle` will toggle the attendance status and is reflected in the UI in the form of the attendance icon change.

==== Current Implementation

The UI now consists of 3 list panels: `PersonListPanel`, `EpicEventListPanel` and `AttendanceListPanel`. Each panel is a list of `PersonCard`, `EpicEventCard` and `AttendanceCard` respectively. Since the `AttendanceCard` has the same UI as `PersonCard` with the addition of the toggle icon, it extends `PersonCard`. The `model` maintains a `selectedEpicEvent` variable whose attendees will be displayed in the `AttendancePanel`.
...
To listen to changes in attendance. We enable the `AttendanceList` to report changes on the element by providing an properties extractor. First, we use the JavaBeans API to represent properties of `Attendance` that we want to listen to, which is the `hasAttendedEventProperty`, a `BooleanProperty` object.
