...
* exposes an unmodifiable `ObservableList<Attendance>` that can be 'observed' e.g. the UI can be bound to this list so that the UI automatically updates when the data in the list change.
...
it is stored inside the `EpicEvent`. The instance stores the corresponding `EpicEvent`, `Person`
